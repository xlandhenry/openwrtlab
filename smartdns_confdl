...
tr_gfwlist()
{
	...
	grep -E -o "(([a-z0-9A-Z-])+\.)+([a-zA-Z]{2,6})" ./gfwlist_decoded.txt | sed 's/^\.//' | sort | uniq -u > ./gfwlist_domains.txt
...
tr_chinalist()
{
	grep -E -o "(([a-z0-9A-Z-])+\.)+([a-z]{2,6})" ./chinalist.txt | sort | uniq -u > ./chinalist_domains.txt
	echo "[$(date "+%Y-%m-%d %H:%M:%S")][INFO] China domains saved to chinalist_domains.txt." >> /var/log/smartdns/smartdns.log
	cp -rf ./chinalist.txt /etc/smartdns/domain-set/china.conf
